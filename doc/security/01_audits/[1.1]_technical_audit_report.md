...
- [x] **Issue:** `db/database.py` initializes the async engine without specifying `pool_size` or `max_overflow`.
...
- [x] **Issue:** In `bot/handlers/photo.py` (lines 67-70), `nutrition_data` values are divided by `weight_g` to get per-gram values. The `weight_g` value comes directly from GPT-4o with no validation.
...
- [x] **Issue:** `api/webhook.py` only exposes `/webhook`. There is no `/health` or `/ready` endpoint.
...
- [x] **Issue:** The `Dockerfile` has no `USER` directive. The application runs as `root` inside the container.
...
- [x] **Issue:** `core/i18n.py` line 89 calls `text.format(**kwargs)`. If a translation template contains `{dish}` but the caller doesn't pass `dish` in kwargs, it throws `KeyError`.
