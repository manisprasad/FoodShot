...
- Python 3.11+
- Poetry
- Cloudflare Tunnel (`cloudflared`) or [ngrok](https://ngrok.com/) (for exposing local webhook)
...
Install dependencies and start the database, Redis, and bot locally:
```bash
task install
task dev
```

### 3. Expose Local Webhook (Cloudflare Tunnel or ngrok)

#### Option A: Cloudflare Tunnel (Recommended)
You can use the built-in Cloudflare Tunnel helper in Taskfile:
```bash
task tunnel
```
Or if you have a persistent tunnel, fill in the `CLOUDFLARE_TUNNEL_TOKEN` in your `.env` file and it will start automatically with `task dev`.

#### Option B: ngrok
In a new terminal window, expose your local port 8000:
```bash
ngrok http 8000
```
